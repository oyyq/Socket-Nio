            ...
            try {
                if (currentFrame.handle(args)) {
                    ...
                    if (currentFrame instanceof ReceiveHeaderFrame) {
                        // Packet 头帧接收完成，则根据头帧信息构建接收的Packet
                        ...
                        completeEntityFrame((ReceiveEntityFrame) currentFrame);          // Packet 实体帧消费完成，则将当前帧消费到Packet
                    }

                    frameTemp = null;          // 接收完成后，直接推出循环，如果还有未消费数据则交给外层调度
                    break;
                }
            } catch (IOException e) {
                //e.printStackTrace();    //如: 把args中的数据写出到packet通道时, 通道已经关闭, 抛出异常, 跳出循环
                break;
            }
        } while (args.remained());
    }
